      ...
      <div className="bg-gradient-to-l from-madrasati-dark via-madrasati-teal to-madrasati-teal-light text-white rounded-3xl p-6 md:p-8 shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
          <div className="flex flex-col md:flex-row items-center gap-6 max-w-3xl w-full text-center md:text-right">
            {/* Principal Photo Card - Enlarged Frame with Direct Change Trigger */}
            <div className="relative shrink-0 select-none my-2 group">
              ...
              <div 
                ...
                className={`relative w-36 h-36 sm:w-44 sm:h-44 md:w-48 md:h-48 lg:w-52 lg:h-52 rounded-3xl overflow-hidden border-4 bg-slate-50 shadow-2xl flex items-center justify-center shrink-0 transition-all duration-300 ${
                  isPhotoDragging ? 'border-emerald-400 ring-4 ring-emerald-300 scale-105' : 'border-white hover:scale-[1.02]'
                }`}
              >
                ...
                {/* Direct Change Overlay */}
                <div 
                  onClick={() => {
                    if (!isAdminMode && onOpenAdminLogin) {
                      onOpenAdminLogin(() => {
                  ...
                  className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white cursor-pointer p-2 text-center backdrop-blur-2xs"
                  title="اضغط لتغيير أو رفع صورة المدير"
                >
                  <LucideIcon name={isCompressingPhoto ? "RefreshCw" : "Camera"} size={26} className={isCompressingPhoto ? "animate-spin mb-1 text-emerald-400" : "mb-1 text-emerald-300"} />
                  <span className="text-xs font-black">{isCompressingPhoto ? 'جارِ المعالجة...' : 'تغيير الصورة'}</span>
                  <span className="text-[9px] text-slate-200">اضغط أو اسحب صورة جديدة</span>
                </div>
              ...
              <div className="absolute -bottom-3 transform translate-x-1/2 right-1/2 bg-white text-slate-900 font-black text-xs py-1.5 px-4 rounded-full border-2 border-teal-500/30 shadow-lg whitespace-nowrap flex items-center gap-1.5 z-10">
            ...
            <div className="space-y-4 flex-1">
              ...
              <p className="text-teal-50/90 text-xs md:text-sm leading-relaxed font-medium">
                المنصة الرقمية لتوثيق المعايير والحوكمة التعليمية بـ {config.schoolName}، ورصد المؤشرات والشواهد الرقمية بفاعلية وتكامل وفق معايير وزارة التعليم بالمملكة العربية السعودية.
              </p>
          ...
          <div className="flex flex-col items-center bg-white/10 p-5 rounded-2xl border border-white/20 backdrop-blur-md w-full md:w-auto min-w-[190px] shrink-0">
            ...
            <p className="text-[11px] text-teal-55 mt-3 text-center">
              استكمال <strong>{metCriteria}</strong> من <strong className="text-white">{totalCriteria}</strong> معياراً
            </p>
          </div>
        </div>
      </div>
