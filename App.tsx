...
export default function App() {
  ...
  const [pages, setPages] = useState<PortfolioPage[]>(() => {
    const saved = localStorage.getItem('portfolio_pages_v6_clean');
    if (saved) {
      try {
    ...
    const legacySaved = localStorage.getItem('portfolio_pages_v1');
    if (legacySaved) {
      try {
        const parsed: PortfolioPage[] = JSON.parse(legacySaved);
        // Clean out default mock criteria & default mock attachments (c1-1, a1-1, etc.)
        return parsed.map((p) => ({
          ...p,
          criteria: p.criteria ? p.criteria.filter(c => !/^c\d+-/.test(c.id)) : [],
          attachments: p.attachments ? p.attachments.filter(a => !/^a\d+-/.test(a.id)) : []
        }));
      } catch (e) {
        return DEFAULT_PAGES;
      }
  ...
  const handleOpenAttachment = (attUrl: string, attName: string) => {
    ...
    openPrompt(
      ...
      `هذا الشاهد [${attName}] محمي بكلمة مرور معتمدة من حساب المدير. يرجى إدخال كلمة المرور للاطلاع عليه:`,
      'أدخل كلمة مرور الشواهد...',
      (inputPassword) => {
        if (inputPassword === attachmentsPassword) {
  ...
  const handlePromptChangeAttachmentsPassword = () => {
    openPrompt(
      ...
      `كلمة المرور الحالية لجميع الشواهد هي [${attachmentsPassword}]. أدخل كلمة المرور الجديدة التي سيتم اعتمادها لجميع الشواهد المرفقة:`,
      'كلمة المرور الجديدة...',
      (newPass) => {
        if (!newPass.trim()) {
        ...
        triggerFeedback('success', `تم تغيير وتأمين كلمة مرور جميع الشواهد إلى: (${newPass.trim()}) بنجاح.`);
      }
    );
  };
  ...
  useEffect(() => {
    async function loadDataFromDb() {
      try {
        const [dbConfig, dbPages] = await Promise.all([
          api.getConfig(),
          api.getPages(),
        ]);
        ...
        if (dbConfig) {
          ...
          // Seed database with current initial config
        ...
        if (dbPages && dbPages.length > 0) {
          ...
          // Seed database with current default pages
          api.savePages(pages);
        }
      } catch (err) {
        console.warn('Initial DB load note:', err);
      } finally {
  ...
  // Save changes to localStorage & Database whenever they mutate
  useEffect(() => {
    localStorage.setItem('portfolio_pages_v1', JSON.stringify(pages));
  ...
  useEffect(() => {
    localStorage.setItem('portfolio_config_v1', JSON.stringify(config));
    if (isInitialDbLoadDone.current) {
      const timer = setTimeout(() => {
        api.saveConfig(config);
      }, 1000);
        ...
        <div className="max-w-7.5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5 justify-end w-full sm:w-auto">
            ...
            <div className="hidden lg:flex items-center gap-2 text-xs bg-slate-50 border border-slate-200/60 px-3.5 py-2 rounded-xl text-slate-600">
              ...
              <span className="font-bold">مؤشر الإنجاز المعتمد:</span>
            ...
            <button
              onClick={() => {
                if (!isAdminMode) {
                  openPrompt(
                    'تفعيل وضع الإدارة والمشرف',
                    'الرجاء إدخال رمز المرور لتفعيل وضع الإدارة وإتاحة صلاحيات الإضافة والحذف والتعديل (الافتراضي: 1234):',
                    'رمز المرور...',
                    (code) => {
                      if (code === "1234" || code === "") {
                        setIsAdminMode(true);
                        triggerFeedback('success', 'تم تفعيل وضع الإدارة والمشرف بنجاح! تم فتح كافة حقول وصلاحيات التعديل.');
                      } else {
                        triggerFeedback('error', 'رمز المرور غير صحيح.');
                      }
                    }
                  );
                } else {
            ...
            >
              ...
              <span>{isAdminMode ? 'وضع الإدارة (نشط)' : 'وضع الزائر (عرض)'}</span>
            </button>
            ...
            {isAdminMode && (
              ...
              >
                ...
                <span className="hidden sm:inline">كلمة مرور الشواهد:</span>
                <span className="bg-amber-200/80 px-1.5 py-0.5 rounded text-[10px] font-black dir-ltr">{attachmentsPassword}</span>
              </button>
            )}
            ...
            <button
              ...
              onClick={() => {
                if (!isAdminMode) {
                  openPrompt(
                    'صلاحيات المشرف المطلوبة',
                    'لوحة التحكم الشاملة تتطلب تفعيل صلاحيات المشرف لتحديث البيانات والهوية. الرجاء إدخال رمز المرور (الافتراضي: 1234):',
                    'رمز المرور...',
                    (code) => {
                      if (code === "1234" || code === "") {
                        setIsAdminMode(true);
                        setIsControlPanelOpen(true);
                        triggerFeedback('success', 'تم تفعيل وضع الإدارة وفتح لوحة التحكم بنجاح.');
                      } else {
                        triggerFeedback('error', 'رمز المرور غير صحيح.');
                      }
                    }
            ...
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between pb-1">
                  ...
                  <span className="text-[10px] md:text-[11px] bg-madrasati-teal-bg px-2.5 py-0.5 rounded-full font-black text-madrasati-teal">فهرس معتمد</span>
                </div>
                      ...
                      >
                        ...
                        <div className="space-y-1 overflow-hidden w-full text-right">
                          ...
                          <div className="flex items-center justify-between text-[11px] text-slate-400">
                            <span className="font-semibold">{page.code ? `رمز: ${page.code}` : `رمز المعيار: إج-0${index + 1}`}</span>
              ...
              <DashboardStats 
                ...
                onToggleControlPanel={() => {
                  if (!isAdminMode) {
                    openPrompt(
                      'صلاحيات المشرف المطلوبة',
                      'لوحة التحكم تتطلب تفعيل صلاحيات المشرف لتعديل الإعدادات والصفحات. الرجاء إدخال رمز المرور (الافتراضي: 1234):',
                      'رمز المرور...',
                      (code) => {
                        if (code === "1234" || code === "") {
                          setIsAdminMode(true);
                    ...
                    <div className="space-y-3.5 z-10 w-full text-right" dir="rtl">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black text-madrasati-teal bg-madrasati-teal-bg px-3 py-1 rounded-full">
                          رمز المعيار: {activePage.code || `إج-0${pages.findIndex(p => p.id === activePage.id) + 1}`}
                        </span>
                      </div>
                        ...
                        <div className="space-y-0.5 text-right">
                          ...
                          <p className="text-[11px] text-slate-500 font-bold">
                            {isAdminMode 
                              ...
                              : 'استعرض المعايير ومؤشرات الأداء المعتمدة لهذا البند.'}
                          </p>
                        </div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 pb-3">
                        <div className="space-y-1 text-right">
                          <div className="flex items-center gap-2 flex-wrap">
                            <h3 className="font-extrabold text-slate-900 text-sm flex items-center gap-2 justify-start">
                              <span className="w-2.5 h-2.5 rounded-full bg-madrasati-teal block"></span>
                              {isAdminMode ? 'ثانياً: الشواهد الرقمية والمرفقات الثبوتية المعتمدة' : 'الشواهد الرقمية والمرفقات الثبوتية المعتمدة لهذا البند'}
                            </h3>
                            {isAttachmentsLockEnabled ? (
                          ...
                          <p className="text-[11px] text-slate-500 font-bold">
                            {isAdminMode 
                              ...
                              : 'استعرض الشواهد والوثائق الرقمية وروابط الحوكمة المعتمدة التي تبرز تميز العمل الإداري والمدرسي لهذه الصفحة.'}
                          </p>
                        </div>
                          ...
                          {isAdminMode && (
                            <>
                              <button
                                ...
                                title="تعديل وتعين كلمة مرور جميع الشواهد"
                              >
                                <LucideIcon name="KeyRound" size={13} className="text-amber-700" />
                                <span>كلمة مرور الشواهد: {attachmentsPassword}</span>
                              </button>
          ...
          <div className="w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col overflow-hidden text-right animate-slide-left">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 bg-slate-100/30 p-3 rounded-xl border border-slate-200/50">
                      ...
                      <div className="space-y-1.5 text-right">
                        ...
                        <div 
                          ...
                          onDrop={(e) => {
                            ...
                            if (file) {
                              ...
                              reader.onload = (event) => {
                                setConfig(prev => ({ ...prev, managerPhotoUrl: event.target?.result as string }));
                                triggerFeedback('success', 'تم رفع وحفظ صورة المدير بنجاح.');
                              };
                        ...
                        >
                          ...
                          <span className="text-[8px] text-slate-400">JPG, PNG (تخزين محلي آمن بالمتصفح)</span>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 text-xs">
                        <div className="flex items-center gap-1.5 shrink-0">
                          ...
                          <input
                            ...
                            title="رمز المعيار حسب الدليل الإجرائي"
                          />
                        </div>
                  ...
                  {sections.map(s => (
                    <div key={s.id} className="flex items-center justify-between p-2.5 bg-slate-50 border border-slate-200/50 rounded-xl">
                      <div className="flex items-center gap-2">
                        ...
                        {s.id === 'school-admin' && (
                          <span className="text-[9px] bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded">الافتراضي المعتمد</span>
                        )}
                      </div>
              ...
              <div className="space-y-3 pt-2">
                ...
                <h4 className="font-bold text-slate-900 text-xs flex items-center gap-2">
                  ...
                  رابعاً: تأمين الحماية والشواهد بكلمة مرور (خاص بقائد المدرسة)
                </h4>
                
                <p className="text-[10px] text-slate-500 font-bold leading-relaxed">
                  يمكنك حماية وتأمين جميع الشواهد والمرفقات المرفوعة بملف الإنجاز بكلمة مرور موحدة. لن يتمكن الزوار أو المستفيدون من فتح أو تحميل أي شاهد إلا بعد إدخال كلمة المرور المعتمدة من قبلكم.
                </p>
                ...
                <div className="p-4 bg-amber-50/60 border border-amber-200/70 rounded-xl space-y-3">
                  ...
                  <div className="pt-2 border-t border-amber-200/50 space-y-2">
                    <label className="text-[10px] font-extrabold text-amber-900 block">
                      كلمة مرور الشواهد المعتمدة لجميع المرفقات:
                    </label>
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          triggerFeedback('success', `تم اعتماد وتحديث كلمة مرور الشواهد إلى: (${attachmentsPassword.trim()})`);
                        }}
                    ...
                    <p className="text-[9px] text-amber-800 font-semibold">
                      * كلمة المرور الافتراضية حالياً: <strong className="font-black bg-amber-200/60 px-1.5 py-0.5 rounded dir-ltr">{attachmentsPassword}</strong>. يمكن للـمدير تعديلها وإعادة ضبطها بأي وقت.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-3 bg-rose-50 border border-rose-100 text-rose-950 rounded-xl space-y-2">
                ...
                <p className="text-[10px] text-rose-700">
                  هل ترغب في إعادة ضبط الموقع، وإرجاع الأقسام الـ 19 للأسماء والصيغ المعتمدة الأساسية بوزارة التعليم؟ هذا الإجراء سيمحو المسودات التي لم تحفظ بالمستودعات.
                </p>
                <div className="pt-1">
                  <button
            ...
            <div className="bg-slate-50 border-t border-slate-100 p-4 flex items-center justify-between gap-2 shrink-0">
              <div className="flex items-center gap-1.5 text-xs text-emerald-700 font-bold">
                ...
                <span>قاعدة البيانات السحابية متصلة وجاهزة</span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={async () => {
                    try {
                      ...
                      setIsControlPanelOpen(false);
                      triggerFeedback('success', 'تم حفظ وتثبيت كافة التعديلات في قاعدة البيانات السحابية بنجاح!');
                    } catch (e) {
                      setIsControlPanelOpen(false);
          ...
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-100 flex flex-col p-6 space-y-4 animate-scale-up text-right">
            ...
            <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-bold">إجراء نظام معتمد بالملف</p>
              </div>
            </div>
      ...
      <footer className="bg-white border-t border-slate-100 py-6 text-center mt-12">
        <div className="max-w-7.5xl mx-auto px-4 space-y-2">
          <p className="text-[11px] text-slate-400 font-bold tracking-wider">
            ملف إنجاز الإدارة والتميز الرقمي المعتمد © {config.year}
          </p>
        </div>
      </footer>
